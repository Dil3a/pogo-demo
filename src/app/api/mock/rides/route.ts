 ...
 * POST /api/mock/rides
 *
 * Reserves a scooter and authorizes payment. Does NOT yet send unlock —
 * the client must call POST /rides/:id/unlock next. This two-step flow lets us
 * fail fast on payment without committing to a hardware action.
 *
...
export async function POST(request: Request) {
  ...
  return withIdempotency(idempotencyKey, async () => {
    ...
    if (method.type === 'student_card') {
      ...
      // Debit immediately (real backend would use a 2-phase: authorize → capture).
      ...
      store.transactions.unshift({
        ...
        relatedRideReference: null, // filled after we know it
      });
    }
    // Card flow would call CMI/Stripe here.
    ...
    // Lock the scooter row.
    scooter.status = 'occupied';
    scooter.stationId = null;

    store.rides.unshift(ride);

    // Link the transaction we just created back to this ride.
